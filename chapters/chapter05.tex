...
A partir del relevamiento competitivo y del análisis de las fuerzas del entorno, se elaboró una matriz FODA para identificar los factores internos y externos que condicionan el desarrollo de GameTrack. Las fortalezas y debilidades describen características propias del proyecto, mientras que las oportunidades y amenazas corresponden a condiciones del contexto. La figura~\ref{fig:foda} presenta una síntesis de los principales factores identificados.

\begin{figure}[H]
	\centering
	\DiagramFig{diagrama-foda.png}
	\caption{Análisis FODA de GameTrack.}
	\label{fig:foda}
\end{figure}

Entre las principales fortalezas se encuentra la propuesta de valor dual de GameTrack. La plataforma combina recomendaciones personalizadas destinadas a los jugadores con herramientas de análisis de reseñas orientadas a los desarrolladores. Esta integración permite abordar dos necesidades relacionadas a partir de una misma base de información: ayudar al jugador a descubrir títulos adecuados y permitir que los estudios comprendan cómo son recibidos sus videojuegos.

Otra fortaleza es la utilización de un sistema híbrido de recomendación. La combinación de filtrado basado en contenido, filtrado colaborativo y un mecanismo de respaldo basado en popularidad permite aprovechar distintas señales y reducir las limitaciones de cada enfoque por separado. A esto se suma la incorporación de datos reales procedentes de Steam y SteamSpy, como metadatos, etiquetas, reseñas, horas jugadas e indicadores de popularidad. La organización modular de la aplicación también facilita el mantenimiento y la evolución independiente de sus componentes, aunque el prototipo se ejecute como un único proceso.

La principal debilidad surge del carácter académico del prototipo. GameTrack no cuenta con una base real y sostenida de usuarios, por lo que el volumen de interacciones internas resulta insuficiente para validar el filtrado colaborativo en condiciones semejantes a las de una plataforma productiva. Esta limitación también dificulta la evaluación completa del sistema de recomendación, ya que algunas métricas requieren historiales de uso, calificaciones y patrones de comportamiento que solamente pueden obtenerse mediante una adopción prolongada.

El proyecto también se encuentra condicionado por el tiempo y los recursos disponibles para el equipo. Determinadas funcionalidades, como la integración social, el contenido audiovisual, el despliegue a gran escala y la evaluación con una población extensa, quedan fuera del alcance del prototipo. Estas restricciones no invalidan la solución, pero limitan el grado en que sus resultados pueden generalizarse a un entorno comercial.

Las oportunidades se relacionan principalmente con la fragmentación de las soluciones existentes. Las plataformas analizadas ofrecen catálogos, reseñas, recomendaciones o información para desarrolladores, pero no reúnen de manera integrada una recomendación híbrida y explicable para el jugador con un análisis de opiniones orientado al estudio responsable del título. Esta ausencia permite diferenciar a GameTrack mediante la centralización de capacidades que actualmente requieren consultar múltiples servicios.

La disponibilidad de APIs y fuentes públicas representa otra oportunidad, ya que facilita la construcción del catálogo y permite enriquecer los videojuegos con metadatos, etiquetas y reseñas reales. Asimismo, la arquitectura y el modelo de datos pueden extenderse en el futuro hacia otras tiendas, plataformas y fuentes de información, reduciendo progresivamente la dependencia de un único ecosistema.

Entre las amenazas se destaca la capacidad de las plataformas consolidadas para incorporar funcionalidades semejantes. Servicios como Steam disponen de una base de usuarios, infraestructura y volumen de datos considerablemente superiores, lo que les permitiría desarrollar recomendaciones más avanzadas o incorporar herramientas adicionales de análisis. La ventaja de GameTrack depende, por lo tanto, de su especialización y de la integración de capacidades que no constituyen actualmente el foco principal de estas plataformas.

También representan una amenaza los posibles cambios en las interfaces y condiciones de acceso de los proveedores externos. La limitación o eliminación de una API puede afectar la actualización del catálogo, la sincronización de bibliotecas y la incorporación de reseñas. A esto se suma la competencia indirecta de asistentes generales de inteligencia artificial, plataformas de análisis de mercado, redes sociales y creadores de contenido, que pueden satisfacer parcialmente la necesidad de descubrir o evaluar videojuegos.

El análisis FODA muestra que GameTrack posee una propuesta diferencial y técnicamente viable, pero se encuentra condicionado por la falta de una base real de usuarios y por la dependencia de fuentes externas. En consecuencia, la estrategia del proyecto debe concentrarse en validar su propuesta de valor, preservar la información obtenida de terceros y demostrar la utilidad de integrar recomendaciones explicables y análisis de reseñas dentro de una misma solución.

\section{Cinco Fuerzas de Porter}\label{sec:porter}

Luego de analizar las plataformas existentes y las capacidades que ofrecen, se aplicó el modelo de las Cinco Fuerzas de Porter con el propósito de estudiar las condiciones competitivas del entorno en el que se inserta GameTrack. El análisis considera como industria de referencia a las plataformas digitales orientadas al descubrimiento de videojuegos, la recomendación personalizada y el análisis de reseñas. La figura~\ref{fig:porter} sintetiza la intensidad estimada de cada fuerza y los factores que la determinan.

\begin{figure}[H]
	\centering
	\DiagramFig{diagrama-porter.png}
	\caption{Cinco Fuerzas de Porter aplicadas a GameTrack.}
	\label{fig:porter}
\end{figure}

La rivalidad entre competidores existentes se considera alta. El mercado cuenta con plataformas consolidadas como Steam, Backloggd y RAWG, que ofrecen catálogos amplios y distintas funcionalidades de descubrimiento, seguimiento y valoración de videojuegos. Estas soluciones poseen comunidades establecidas, reconocimiento de marca y grandes volúmenes de información. En este contexto, GameTrack no competiría únicamente por ofrecer un catálogo, sino principalmente por captar y mantener la atención de los usuarios. Su diferenciación se encuentra en la combinación de recomendaciones híbridas, explicaciones asociadas a cada sugerencia y análisis de reseñas orientado a desarrolladores, capacidades que actualmente aparecen distribuidas entre distintas plataformas.

La amenaza de nuevos entrantes presenta una intensidad media. Desde una perspectiva técnica, existen barreras de entrada relativamente bajas debido a la disponibilidad de APIs, frameworks de desarrollo y bibliotecas de aprendizaje automático. Estos recursos permiten construir prototipos y servicios de recomendación sin requerir inicialmente una infraestructura de gran escala. Sin embargo, desarrollar una plataforma funcional no garantiza su adopción. La construcción de una comunidad, la generación de un volumen suficiente de interacciones y la obtención de datos útiles para alimentar el filtrado colaborativo constituyen barreras más difíciles de superar. Los productos establecidos se benefician de efectos de red: a medida que aumenta la cantidad de usuarios, también crece la información disponible para mejorar las recomendaciones y la experiencia general.

El poder de negociación de los proveedores se considera alto debido a la dependencia de fuentes externas. GameTrack obtiene metadatos, reseñas, etiquetas e información de popularidad mediante servicios como Steam y SteamSpy. Estos proveedores pueden modificar sus interfaces, limitar la frecuencia de las solicitudes, cambiar sus condiciones de uso o dejar de ofrecer determinados datos. La plataforma no posee control sobre estas decisiones, por lo que una alteración externa puede afectar los procesos de incorporación y actualización del catálogo. Esta condición justifica el almacenamiento local de la información obtenida y la diferenciación entre la ausencia real de un dato y una falla temporal de comunicación.

El poder de negociación de los compradores también se considera alto en el segmento de jugadores. Los usuarios disponen de numerosas alternativas gratuitas y pueden cambiar de plataforma sin afrontar costos significativos. Esto incrementa sus expectativas respecto de la precisión de las recomendaciones, la cantidad de información disponible y la facilidad de uso. La plataforma debe demostrar rápidamente un valor diferencial para evitar que el usuario regrese a soluciones conocidas. En el segmento de desarrolladores existe una menor cantidad de herramientas que combinen el análisis de reseñas con el entorno de descubrimiento destinado a jugadores, lo que reduce parcialmente este poder, aunque permanecen disponibles servicios especializados de inteligencia de mercado y análisis de opiniones.

Finalmente, la amenaza de productos sustitutos es alta. La necesidad de descubrir y evaluar videojuegos puede satisfacerse parcialmente mediante tiendas digitales, sitios de reseñas, redes sociales, videos, transmisiones en vivo, recomendaciones de amigos y asistentes generales de inteligencia artificial. Para los desarrolladores también existen plataformas de análisis de mercado y herramientas especializadas en procesamiento de opiniones. Aunque ninguno de estos sustitutos reúne necesariamente todas las capacidades de GameTrack, su combinación permite al usuario obtener información semejante. Por este motivo, el valor de la propuesta depende de centralizar esas capacidades, reducir el esfuerzo de búsqueda y ofrecer resultados personalizados y explicables.

En conjunto, el análisis muestra un entorno competitivo exigente, caracterizado por una rivalidad elevada, múltiples sustitutos, usuarios con bajos costos de cambio y una dependencia significativa de fuentes externas. La viabilidad diferencial de GameTrack no reside en competir por tamaño de catálogo, sino en integrar recomendaciones personalizadas para jugadores y análisis de opiniones para desarrolladores dentro de una misma plataforma.
...
\begin{figure}[ht]
	\centering
	\DiagramFig{diagrama-casos-uso.png}
...
\begin{figure}[H]
	\centering
	\WideDiagramFig{arquitectura-despliegue.png}
...
\begin{figure}[H]
	\centering
	\WideDiagramFig{diagrama-clases.png}
...
\begin{figure}[H]
	\centering
	\WideDiagramFig{diagrama-er.png}
