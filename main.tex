...
\newpagestyle{ruled}{
	\sethead{\includegraphics[width=3.7cm]{./images/UADE_LARGE}}
			{\parbox[b]{0.43\textwidth}{\raggedright\MakeUppercase{Plataforma inteligente de recomendación y análisis de videojuegos}}} % El parámetro 0.43 puede modificarse para tener más espacio en la columan del medio, y que así el título de la tesis entre mejor.
			{
				\raisebox{1.5ex}{%
...
% Numeraci\'on de tablas: cap\'{\i}tulo.n\'umero (ej. Tabla 5.1)
\renewcommand{\thetable}{\thechapter.\arabic{table}}

% Para tablas de ancho fijo con ajuste de p\'arrafo
\usepackage{tabularx}

% Placeholder para capturas de pantalla y diagramas a\'un no incorporados como archivo de imagen.
% Reemplazar el \includegraphics correspondiente por la imagen real cuando est\'e disponible.
\newcommand{\PlaceholderFig}[1]{%
	\begin{framed}
		\centering
		\textit{[Insertar imagen: #1]}
	\end{framed}
}
\newcommand{\ScreenshotFig}[1]{%
	\includegraphics[width=\textwidth]{./images/#1}%
}
\newcommand{\SurveyFig}[1]{%
...
\newcommand{\DiagramFig}[1]{%
	\IfFileExists{./images/#1}{%
		\includegraphics[width=\textwidth,height=.78\textheight,keepaspectratio]{./images/#1}%
	}{%
		\PlaceholderFig{#1}%
	}%
}
\newcommand{\WideDiagramFig}[1]{%
	\IfFileExists{./images/#1}{%
		\includegraphics[width=\linewidth,height=.78\textheight,keepaspectratio]{./images/#1}%
	}{%
		\PlaceholderFig{#1}%
	}%
}
...
\clearpage
	...
	\input{chapters/appendix/schedule_of_activities}
